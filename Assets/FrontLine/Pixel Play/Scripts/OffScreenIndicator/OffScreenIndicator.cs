...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void DrawIndicators()
    {
        ...
        {
            OnScreenObjectPool.current.DeactivateAllPooledObjects();
            OffScreenObjectPool.current.DeactivateAllPooledObjects();
            return;
        }
        foreach(Target target in targets)
        {
            ...
            if(target.NeedOnScreenIndicator && isTargetVisible)
            {
                screenPosition.z = 0;
                indicator = GetIndicator(ref target.indicator, IndicatorType.OnScreen); // Gets the box indicator from the pool.
            }
            else if(target.NeedOffScreenIndicator && !isTargetVisible)
            {
                float angle = float.MinValue;
                OffScreenIndicatorCore.GetArrowIndicatorPositionAndAngle(ref screenPosition, ref angle, screenCentre, screenBounds);
                indicator = GetIndicator(ref target.indicator, IndicatorType.OffScreen); // Gets the arrow indicator from the pool.
                indicator.indicatorImage.transform.rotation = Quaternion.Euler(0, 0, angle * Mathf.Rad2Deg); // Sets the rotation for the arrow indicator.
            }
            if(indicator)
            {
    ...
    private Indicator GetIndicator(ref Indicator indicator, IndicatorType type)
    {
        if(indicator != null)
        {
            if(indicator.Type != type)
            {
                ...
                indicator = type == IndicatorType.OnScreen ? OnScreenObjectPool.current.GetPooledObject() : OffScreenObjectPool.current.GetPooledObject();
